...
class Player{
	...
	initFields(items){
		...
		const yashiki = this.items.getItemByName("密偵");
		for(let i=0;i<3;i++){
			this.pile.push(yashiki);
		}
		/*
		実装あとまわし
		呪い,玉座の間,役人,書庫,魔女,冒険者
		*/
	...
	*getRestrictedFieldChoice(f){
	...
	refleshPlayer(){
		...
		if(Utils.getOption('debugitem').ison){
			console.log('Player ' + this.playeridx);
			['pile','hand','discards','nowdiscards'].forEach((s)=>{
 				console.log(s,this[s].map((c)=>{return c.name}));
			});
		}
	}
